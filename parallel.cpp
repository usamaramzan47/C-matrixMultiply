...
void createRandomMatrices(int rows, int cols, int ***matrixA, int ***matrixB) {
	clock_t start, end;
	start = clock();
...
#pragma omp section
	...
	printf("\nTime taken to generate matrices: %f seconds\n", cpu_time_used);


	FILE *file = fopen("TimeTaken.txt", "a");
	if (file == NULL) {
		printf("Failed to open the file for writing.\n");
		return;
	}
...
int main() {
	...
	*/
	...
	printf("Enter the number of rows for martrix-1: ");
	...
	if (choice == 1) {
		...
		createRandomMatrices(numRowsMatrixA, numColsMatrixA, &matrixA, &matrixB);
